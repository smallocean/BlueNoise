#include <iostream>
#include <vector>
using namespace std;

template<class T>
class Array
{
public:
    Array(void);
    Array(const vector<int> & spec);
    virtual ~Array(void);

    int Dimension(void) const;    //Î¬Êý
    int Size(const int which_dim) const;//Í¼Ïñ´óÐ¡
    void Size(vector<int> & answer) const;

    // both return 1 if operation successful, 0 else
    int Get(const vector<int> & index, T & output) const;
    int Put(const vector<int> & index, const T & input);

    // batch mode, handle all
    int Get(vector<T> & output) const;
    int Put(const vector<T> & input);
    
    void Clear(const T & value);
    
protected:
    int ConvertIndex(const vector<int> & index) const;
    
protected:
    vector<int> _size;
    vector<T> _data;
};

template<class T>
Array<T>::Array(void)
{
    
}

template<class T>
Array<T>::Array(const vector<int> & spec) : _size(spec)
{
    int total_count = 1;
    
    for(unsigned int i = 0; i < _size.size(); i++)
    {
        if(_size[i] <= 0)
        {
            _size[i] = 1;
        }

        total_count *= _size[i];
    }

    _data = vector<T>(total_count);
}

template<class T>
Array<T>::~Array(void)
{
    // nothing to do
}

template<class T>
int Array<T>::Dimension(void) const
{
    return _size.size();
}

template<class T>
int Array<T>::Size(const int which_dim) const
{
    if((which_dim >= 0) && (static_cast<unsigned int>(which_dim) < _size.size()))
    {
        return _size[which_dim];
    }
    else
    {
        return -1;
    }
}

template<class T>
void Array<T>::Size(vector<int> & answer) const
{
    answer = _size;
}

template<class T>
int Array<T>::Get(const vector<int> & index_input, T & output) const
{
    const int index = ConvertIndex(index_input);

    if((index >= 0) && (static_cast<unsigned int>(index) < _data.size()))
    {
        output = _data[index];
        return 1;
    }
    else
    {
        return 0;
    }
}

template<class T>
int Array<T>::Put(const vector<int> & index_input, const T & input)
{
    const int index = ConvertIndex(index_input);
    
    if((index >= 0) && (static_cast<unsigned int>(index) < _data.size()))
    {
        _data[index] = input;
        return 1;
    }
    else
    {
        return 0;
    }
}

template<class T>
int Array<T>::Get(vector<T> & output) const
{
    output = _data;
    return 1;
}

template<class T>
int Array<T>::Put(const vector<T> & input)
{
    if(input.size() == _data.size())
    {
        _data = input;
        return 1;
    }
    else
    {
        return 0;
    }
}

template<class T>
void Array<T>::Clear(const T & value)
{
    for(unsigned int i = 0; i < _data.size(); i++)
    {
        _data[i] = value;
    }
}

template<class T>
int Array<T>::ConvertIndex(const vector<int> & index) const
{
    if(index.size() != _size.size())
    {
        return -1;
    }
    else
    {
        {
            for(unsigned int i = 0; i < index.size(); i++)
            {
                if((index[i] < 0) || (index[i] >= _size[i]))
                {
                    return -1;
                }
            }
        }

        int answer = 0;

        for(unsigned int i = 0; i < index.size(); i++)
        {
            answer *= _size[i];
            answer += index[i];
        }

        return answer;
    }
}